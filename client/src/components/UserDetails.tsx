...
type UserDetailsProps = { 
    name?: string, time: number | null, color: string, myTurn:boolean,onResign?:any,offerDraw?:any,
    socket?:WebSocket,gameStart:boolean
}

const UserDetails = ({ name, time, color,onResign,offerDraw ,myTurn,socket}: UserDetailsProps) => {
    ...
    return (
        <div className="flex flex-row items-center justify-between gap-x-5 w-full bg-zinc-700 text-white p-2">
            <div className="flex items-center gap-x-5">
                ...
                <p className="font-serif font-semibold text-xl">{name ? name : user?.name}</p>
